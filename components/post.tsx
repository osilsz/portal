//
// in the downside if api change or any issues happen i  have also uncomment the drafpost file html css code so if need use that
//
    ...
    <>
      {postContent?.map((item) => {
        return (
          <Card
            key={item.id}
            className=" w-full bg-white-w100 border  border-l-gray-G900 shadow-none px-[16.43px] mt-2"
          >
            <CardHeader className="w-full p-0 flex flex-row justify-between mt-[14.23px] ">
              <div className=" flex">
                {item?.company && !item?.image && !info?.image && (
                ...
                <div className="ml-[11px]">
                  <h2 className="roboto-normal-black">{item?.name}</h2>
...
const cards = [
  ...
  {
    image: cardone,
    title: "And one more card for the carousel.",
  },
];
...
const postContent = [
  {
    ...
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed luctus, augue eget scelerisque efficitur.",
    image: null,
    name: "Your Brand",
    follower: "26.548 followers",
    Promoted: true,
    card: cards,
    carousel: true,
    company: true,
  ...
  },
  {
    ...
    Promoted: false,
    carousel: false,
    card: { image: cardone, title: "price : 400" },
    company: false,
  },
];
